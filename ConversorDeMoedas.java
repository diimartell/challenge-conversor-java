...
public class ConversorDeMoedas extends JFrame {
	...
	public static void main(String[] args) {
		...
		do {
			...
			String n = (String)JOptionPane.showInputDialog(frame, "Escolha uma opção:", "Menu", JOptionPane.PLAIN_MESSAGE, null, opcoes, opcoes[0]);
			if (n == null) {
				System.exit(0);
			}
			...
			String[] opcoes2 = {"De Reais a Dólares", "De Reais a Euros", "De Reais a Libras", "De Reais a Yens", "De Reais a Won Coreano", "De Dólares a Reais", "De Euros a Reais", "De Libras a Reais", "De Yens a Reais", "De Won Coreano a Reais"};
			...
			do {
				...
				if (input.matches("^[Z0-9]*$")) {
						...
						JOptionPane.showMessageDialog(frame, "Por favor, entre apenas com números positivos", "Por favor, entre apenas com números positivos", JOptionPane.ERROR_MESSAGE);
						JOptionPane.showMessageDialog(frame, String.format("Conversão de reais para dólares: %.2f", valor), "Conversor", JOptionPane.PLAIN_MESSAGE);
				}	
			} while (!input.matches("^[Z0-9]*$"));
			
			if(p.equals("De Reais a Dólares")) {
			...
			} else if(p.equals("De Dólares a Reais")) {
			...
			if (opcao == JOptionPane.YES_OPTION) {
				...
				JOptionPane.showMessageDialog(frame, "Programa concluído");
				continua = false;
				System.out.println(continua);
				System.exit(0);
			}
	...
	public static double reais_para_won_coreano(double valor) {
		double cotacao_won_coreano = 247.92;
	...
	public static double dolares_para_reais(double valor) {
		double cotacao_real = 5.28;
		valor = valor * cotacao_real;
		JOptionPane.showMessageDialog(frame, String.format("Conversão de dólares para reais: %.2f", valor), "Conversor", JOptionPane.PLAIN_MESSAGE);
		return valor;
	}
	
	public static double euros_para_reais(double valor) {
		double cotacao_real = 5.69;
		valor = valor * cotacao_real;
